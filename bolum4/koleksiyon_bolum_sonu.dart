...
main(List<String> args) {
  ...
  //2- Keyleri string, değerleri dynmaic olan bir map olusturun. Bu map yapısında bilgisayarlarınızın işlemci çekirdek sayısını, ram miktarını ve ssd olup olmadıgını bilgisini tutun ve ekrana yazdırın.
  ...
  //3- Her bir elemanında keyleri string, value'leri dynamic map olan bir liste olusturun. Bu listedeki her bir eleman il adını, ilçe sayısını, plaka kodunu tutsun. Sonrasında bu listeyi okunaklı bir şekilde yazdırın Örnek listenin 1. elemanında bulunan il ankara, plaka kodu:06, ilçe sayısı:10(degerler rsatgele olabilir)
  ...
  //4- 5 elemanlı iki farklı liste olusturun. Elemanlar 0-50'ye rastgele şekilde olusturulsun. Bu elemanları tek bir listeye aktarın. Oluşan son listenin elemanlarının karelerini tutan set yapısı olusturulup ekrana yazdırın.
  List<int> liste1 = List.filled(5, 0);
  var liste2 = List<int>.filled(5, 0);
  List<int> sonListe = <int>[];
  Set<int> sonSetyapisi = <int>{}; //Set elemanları tekrar etmeden saklar
  ...
  //5- Kullanıcıdan aldıgınız integer pozitif sayıları bir listede tutun, kullanıcı -1 değerini gidriğinde girilen sayıların ortalamasını ekrana yazdırın.

  int girilenNot = 0;
  List<int> girilenNotlar = <int>[];
  // List<int> girilenNotlar2 = List.empty(growable: true);  yukarıdaki ile aynı
  do {
